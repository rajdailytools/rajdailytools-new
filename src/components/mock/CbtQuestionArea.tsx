...
import { Bookmark, ChevronLeft, ChevronRight, RotateCcw, Check, Sparkles, LayoutGrid, CheckCircle2 } from 'lucide-react';
...
export const CbtQuestionArea: React.FC<CbtQuestionAreaProps> = ({
  ...
  return (
    <div id="cbt-question-area" className="flex-1 flex flex-col space-y-4">
      ...
      <div className="lg:hidden flex items-center justify-between p-3 rounded-2xl bg-white border border-slate-200 shadow-xs">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-blue-50 hover:bg-blue-100 text-blue-700 text-xs font-bold transition-colors cursor-pointer"
        >
      ...
      {/* Main Question Card */}
      <div id="cbt-active-question-card" className="bg-white border border-slate-200 rounded-3xl p-5 sm:p-8 shadow-xs space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-2.5 pb-4 border-b border-slate-100">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="px-2.5 py-0.5 rounded-full bg-indigo-50 text-indigo-700 text-[11px] font-semibold border border-indigo-100 flex items-center gap-1">
              <Sparkles className="w-3 h-3" />
              <span>{question.sourceLabel || 'RajDailyTools Practice Question'}</span>
            </span>
          </div>
      ...
      <div id="cbt-navigation-bar" className="bg-white border border-slate-200 rounded-2xl p-4 shadow-xs flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            type="button"
            id="btn-cbt-clear-response"
            onClick={onClearResponse}
            disabled={!selectedOption}
            className={`flex items-center gap-1 px-3.5 py-2.5 rounded-xl border text-xs font-bold transition-all cursor-pointer ${
              !selectedOption
        ...
        <div className="flex flex-wrap items-center gap-2 ml-auto">
          <button
            ...
            className={`flex items-center gap-1.5 px-4 py-2.5 rounded-xl border text-xs sm:text-sm font-bold transition-all active:scale-95 cursor-pointer ${
              isMarkedForReview
